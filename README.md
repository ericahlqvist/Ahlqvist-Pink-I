...
2. The folder "Massey" contains a C program built on PARI which computes cup products (in parallell) and computes certain 3-fold Massey products in the étale cohomology of the ring of integers of a number field ![equation](https://latex.codecogs.com/svg.image?K). Since the program was mainly used in the case ![equation](https://latex.codecogs.com/svg.image?K) is imaginary quadratic (i.e., cup products vanish), we have commented out the cup products part. In this case, the program also outputs generating relations for the group ![equation](https://latex.codecogs.com/svg.image?G_K/D_4(G_K)) where ![equation](https://latex.codecogs.com/svg.image?G_K) is the Galois group of the maximal unramified pro-![equation](https://latex.codecogs.com/svg.image?p)-extension of ![equation](https://latex.codecogs.com/svg.image?K). 

3. The folder "SmallGroupIds" contains:
    - "discriminants_3_cyc_[3^m,3^n]"; a file listing a discriminants ![equation](https://latex.codecogs.com/svg.image?(0 > d > -10^8)) whose class group modulo 3 has rank 2 as a vector space over the field with three elements. This was obtained from LMFDB and filtered using a simple Python script.
    ...
    - "gap-p-3-cyc-[3^m,3^n].ipynb"; a notebook taking "all_rels_20260106a" as input to compute the small group ids of the groups ![equation](https://latex.codecogs.com/svg.image?G_K/D_4(G_K)) and compare the frequencies to the Pink–Rubio heuristics. 
